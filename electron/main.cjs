...
let   appPort    = 5001;   // resolved to first free port at startup
const PUSH_PORT  = 9876;

// ── Find a free TCP port ──────────────────────────────────────────────────────
function findFreePort(start = 5001, end = 5020) {
  return new Promise((resolve, reject) => {
    const net = require('net');
    let port  = start;
    function tryNext() {
      if (port > end) return reject(new Error(`No free port found between ${start} and ${end}.`));
      const server = net.createServer();
      server.once('error', () => { port++; tryNext(); });
...
// ── Write startup log to userData so users can share it ──────────────────────
let logStream = null;
...
// ── Health probe ─────────────────────────────────────────────────────────────
function waitForAPI(maxAttempts = 120, onTick) {
  return new Promise((resolve, reject) => {
    let attempts = 0;
    function probe() {
      attempts++;
      ...
      const req = http.get(`http://localhost:${appPort}/api/health`, (res) => {
      ...
      function schedule() {
        if (attempts >= maxAttempts) {
          return reject(new Error(
            `Server did not become ready after ${maxAttempts} seconds.\n\n` +
            `Possible causes:\n` +
            `• Antivirus is scanning mongod.exe — add School Manager to exclusions and restart\n` +
            `• Low memory — close other applications and try again\n\n` +
...
async function startServer(mongoUri) {
  // Pick a free port — avoids "port already in use" on client machines
  appPort = await findFreePort(5001, 5020);
  console.log(`[Server] Using port ${appPort}`);

  return new Promise((resolve, reject) => {
    // Resolve server entry — works for all build modes:
    //   asar: false  → resources/app/server/server.js  (__dirname/../server)
    //   asar: true   → resources/app.asar.unpacked/server/server.js
    //   dev          → project root /server/server.js
    const fromDirname   = path.join(__dirname, '..', 'server', 'server.js');
    const fromUnpacked  = path.join(process.resourcesPath, 'app.asar.unpacked', 'server', 'server.js');
    const serverEntry   = fs.existsSync(fromDirname) ? fromDirname : fromUnpacked;
    ...
    serverProc.on('exit', (code) => {
      if (!resolved && code !== null && code !== 0) {
        reject(new Error(
          `Server process crashed (exit code ${code}).\n\n` +
    ...
    const onTick = (attempt) => {
      ...
      if (elapsed <= 10) {
        ...
        splashStatus(`Starting application server… (${elapsed}s)`);
      } else {
        splashStatus(`First-time setup in progress… ${elapsed}s\nAntivirus scan may be running — please wait`);
      }
    };

    waitForAPI(120, onTick)
...
function registerIPC() {
  ...
  function ensureZKTeco() {
    if (!zkteco) zkteco = new ZKTecoBridge(appPort);
    return zkteco;
  }
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    // 2. Express server (picks a free port automatically)
    splashStatus('Starting application server…');
    ...
    // 4. Auto-start ZKTeco PUSH listener
    zkteco = new ZKTecoBridge(appPort);
    ...
    // Close splash so nothing is left on screen during the error dialog
    try { splashWindow?.destroy(); splashWindow = null; } catch (_) {}
    ...
    // Force everything down — don't rely on before-quit for fatal errors
    app.isQuitting = true;
    try { serverProc?.kill('SIGKILL'); } catch (_) {}
    try { await mongoMgr?.stop(); } catch (_) {}
    try { logStream?.end(); } catch (_) {}
    app.exit(1);   // exit(1) skips before-quit and closes immediately
  }
});
